...
// StoreManager
//
//  Created by Cheng Xian on 8/29/14.
...
@interface FMAAddCardVC ()

@end

@implementation FMAAddCardVC

// -------------------------------------------------------------------------------------------------------------------------
#pragma mark - Basic Functions
- (void)printLogWith:(NSString *)logMessage
{
    if (!debug || !debugFMAAddCardVC) return;
...
//    scanViewController.appToken = [FMAUtil cardIOAppToken];
...
- (IBAction)onBtnSave:(id)sender
{
    ...
    if (![self checkValid])
    {
        [[FMAUtil generalAlertWithTitle:nil message:CHECK_ERROR_MESSAGE delegate:self] show];
        return;
    }
    
    PKCard *card = [self card];
    
    [FMAERegisterUtil storeCreditCard:card];

...
// StoreManager
//
//  Created by Cheng Xian on 8/15/14.
...
@implementation FMAUtil

// --------------------------------------------------------------------------------------------------------------
#pragma mark - Basic Functions
+ (void)printLogWith:(NSString *)logMessage
{
    if (!debug || !debugFMAUtil) return;
...
{
    return [self infoDictionaryValueByKey:@"CARDFLIGHT_API_KEY"];
}
+ (NSString *)cardFlightAccountToken
{
    return [self infoDictionaryValueByKey:@"CARDFLIGHT_ACCOUNT_TOKEN"];
}
...
+ (void)setupInputAccessoryViewWithButtonTitle:(NSString *)buttonTitle selector:(SEL)selector target:(id)target forControl:(id)textControl
{
    ...
    [FMAThemeManager decorateInputAccessoryViewBarButton:barButton textColor:nil];
                                           ...
                                           selectorForDoneButton:(SEL)selectorDoneButton
{
    ...
    [FMAThemeManager decorateInputAccessoryViewBarButton:barButtonPrev withSelector:selectorPreviousButton];
    [FMAThemeManager decorateInputAccessoryViewBarButton:barButtonNext withSelector:selectorNextButton];
                                      ...
                                      selectorForCancelButton:(SEL)selectorCancelButton
{
    ...
    [FMAThemeManager decorateInputAccessoryViewBarButton:barButtonSet    withSelector:selectorSetButton];
    [FMAThemeManager decorateInputAccessoryViewBarButton:barButtonCancel withSelector:selectorCancelButton];
...
{
    FMAWeekdayPickerView *weekdayPicker = [[FMAWeekdayPickerView alloc] initWithFrame:CGRectNull];
    weekdayPicker.weekdayPickerViewDelegate = delegate;
    
    textControl.inputView = weekdayPicker;
...
+ (NSString *)randomStringWithLength:(int)len
{
    ...
    for (int i=0; i<len; i++)
    {
        NSUInteger k = arc4random_uniform((uint32_t)[letters length]) % [letters length];
        [randomString appendFormat: @"%C", [letters characterAtIndex: k]];
    }
...
+ (BOOL)checkAllFieldsExist:(NSArray *)infoFields
{
    ...
    {
        UITextField       *tf = object[kFMAValidationControlKey];
        
        if ([self isStringEmpty:tf.text])
        {
            NSString *placeholder = object[kFMAValidationPlaceholderKey];
            
            tf.text = @"";
            [FMAThemeManager setPlaceholder:placeholder toTextField:tf color:COMMON_COLOR_FOR_ERROR_TITLE];
...
+ (BOOL)checkAndFormatDecimalNumberField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    {
        // When entered "."
        if ([FMAUtil stringContainsString:textField.text Needle:@"."] ||
...
+ (NSInteger)monthFromDate:(NSDate *)aDate
{
    ...
    NSInteger month = [[dateFormatter stringFromDate:aDate] integerValue];
    return month;
}
...
{
    BOOL isLeapYear = [self isYearLeapYear:[NSDate date]];
...
{
    id      contentVC = [FMAUtil instantiateViewControllerBySBID:contentVC_SBID];

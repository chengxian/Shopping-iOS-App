...
// StoreManager
//
//  Created by Cheng Xian on 8/18/14.
...
@implementation FMAThemeManager

// ----------------------------------------------------------------------------------------------------------------
#pragma mark - Basic Functions
+ (void)printLogWith:(NSString *)logMessage
{
    if (!debug || !debugFMAThemeManager) return;
...
+ (UIImage *)blurImageForMainPagesFromImage:(UIImage *)originalImage atFrame:(CGRect)frame
{
    ...
    UIColor *tintColor = RGBHEX(0x000000, 0.3f);
...
+ (UIImage *)blurImageForMainPageToolbarByImage:(UIImage *)image
{
    ...
    CGSize screenSize = [FMAUtil screenRect].size;
...
{
    CGSize menuSize = [[FMAUtil appDelegate].sideMenuVC menuSize];
    CGRect       frame = CGRectMake(0, 0, menuSize.width, image.size.height);
    
    UIColor *tintColor = RGBHEX(0xffffff, .5f);
    UIImage       *res = [image applyBlurWithRadius:30 iterationsCount:3
                                                  tintColor:tintColor saturationDeltaFactor:1.8
                                                  maskImage:nil atFrame:frame];
...
{
    [FMAThemeManager makeTransparentNavigationBar:vc.navigationController.navigationBar];
    
    if (toolbar)
    {
        [FMAThemeManager makeTransparentToolBar:toolbar];
    }
    
    PFImageView *imageviewBackground;
    imageviewBackground = [[PFImageView alloc] initWithFrame:[FMAUtil screenRect]];
...
{
    UIImage *blurredImage = [[FMABackgroundSetting sharedInstance] blurImageForBackgroundName:backgroundName];
...
{
    UIImage *image = [[FMABackgroundSetting sharedInstance] blurImageForBackgroundName:backgroundName];
...
{
    CGRect screenRect = [FMAUtil screenRect];

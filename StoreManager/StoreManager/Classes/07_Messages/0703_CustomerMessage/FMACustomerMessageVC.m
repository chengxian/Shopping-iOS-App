...
@interface FMACustomerMessageVC ()

@end

@implementation FMACustomerMessageVC
{
...
{
    if (!debug || !debugFMACustomerMessageVC) return;
...
- (void)initNavigationBar
{
    ...
    [FMAThemeManager makeCircleWithView:_btnCustomerAvatar
                            borderColor:RGBHEX(0xffffff, 1.f)
                            borderWidth:1.f];
    [FMAThemeManager makeCircleWithView:_btnReceiver
                            borderColor:RGBHEX(0xffffff, 1.f)
                            borderWidth:1.f];
}
...
- (void)sendRequest
{
    ...
    [FMACustomerMessageUtil requestGetMessagesInCustomerMessagePage:[dataSource count]  other:_other delegate:self];
}

- (void)requestGetMessagesInCustomerMessagePageDidRespondWithMessages:(NSArray *)messages
{
    [self printLogWith:@"requestGetMessagesInCustomerMessagePageDidRespondWithMessages"];
...
- (void)addMessagesFromObjects:(NSArray *)messageObjects
{
    ...
    {
        NSString     *senderId = [FMAEmployeeMessageUtil senderIDFromMessage:m];
...
- (void)sendRequestForLatestMessages:(id)sender
{
    ...
    [FMACustomerMessageUtil requestGetLatestMessagesInCustomerMessagePage:lastDate other:_other delegate:self];
...
{
    [self printLogWith:@"requestGetLatestMessagesInCustomerMessagePageDidRespondWithMessages"];
...
- (IBAction)onBtnCall:(id)sender
{
    ...
    if (![FMAUtil doCallPhoneNumber:phoneNumber])
    {
        [[FMAUtil generalAlertWithTitle:ALERT_TITLE_WARNING message:ALERT_MSG_NO_AVAILABLE_PHONE_CALL delegate:self] show];
    }
}
